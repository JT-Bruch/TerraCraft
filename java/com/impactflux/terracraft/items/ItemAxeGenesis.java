...
public class ItemAxeGenesis extends TerraItemAxeAdv
{

	public ItemAxeGenesis(ToolMaterial toolMaterial) 
	{
		super(toolMaterial);
	...
	public boolean onBlockDestroyed(ItemStack stack, World world, Block block, int x, int y, int z, EntityLivingBase entity) {
		...
		if( !TerraItems.genesisEnableFeature[TerraItems.AXE] )
		{
			return true;
		}
	        ...
	        for (int xPos = x - xRange; xPos <= x + xRange; xPos++)
	        {
	            for (int yPos = y - yRange; yPos <= y + yRange; yPos++)
	            {
	                for (int zPos = z - zRange; zPos <= z + zRange; zPos++)
	                {
	                	...
	                	if( bVal )
	                	{	
	                		if( effectChance <= TerraItems.genesisFeatureChance[TerraItems.AXE]  )
	                		{
